...
class Resistance {
    ...
    vector<double> spyProbability(int P, int S, vector<string> missions) {
        int N = missions.size();
        vector<int> ans = vector<int>(P, 0);
        cout << P << " " << S << endl;
        int ok_count = 0;

        REP(mask, 1 << P) {
            int cnt = 0;
            REP(i, P) cnt += ((mask >> i) & 1);
            if (cnt != S) continue;
            bool okok = true;
            ...
            if (okok) {
                ok_count += 1;
                REP(j, P) if (mask & (1 << j)) ans[j] += 1;
            }
        }

        if (ok_count == 0) return vector<double>();

        vector<double> ans_p = vector<double>(P, 0);
        REP(i, P) ans_p[i] = 1.0 * ans[i] / ok_count;
        return ans_p;
    }
};
