...
class TreeDistanceConstruction {
    ...
    vector<int> construct(vector<int> d) {
        ...
        if (d.size() == 1)
            ...
            return (d[0] * d[1] == 1) ? vector<int>(2, 1) : vector<int>();

        int maxval = *max_element(d.begin(), d.end());
        int minval = *min_element(d.begin(), d.end());
        vector<vector<int> > level_node(maxval);

        for (int i = 0; i < d.size(); i++) {
            level_node[d[i]].push_back(i);
        }
        ...
        for (int i = minval+1; i <= maxval; i++) {
            if (d[i].size() < d[i-1].size() && i != maxval)
                return vector<int>();
