...
long long dfs1(int n, int prev) {
    M[n] += 1;
    for (auto m: edges[n]) {
        if (m != prev) M[n] += dfs1(m, n);
    }
    return M[n];
}

long long dfs2(int n, int prev) {
    long double ret = W[n];
}

long long dfs3(int n, int prev) {
...
class AverageVarianceSubtree {
    ...
    double average(vector<int> p, vector<int> weight) {
        REP(i, p.size()) {
            edges[i+1].push_back(p[0]);
            edges[p[0]].push_back(i+1);
        }
        REP(i, weight.size) {
            W[i] = weight[i];
        }
